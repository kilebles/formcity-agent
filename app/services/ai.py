...
ПРАВИЛА ПОДСЧЁТА:
- "Сколько продано", "сколько сделок" → count_values по столбцу "Дата ДДУ" или "Дата договора уступки" (non-null = продана)
- "Сколько квадратных метров продано", "общая площадь" → sum_column по столбцу "Площадь по ДДУ" (в листах "Апартаменты (проект)", "Уступки ВЕЛЛ" и др.); передавай date_column для фильтрации по периоду
