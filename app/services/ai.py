...
ПОДСКАЗКИ ПО ДАННЫМ:
- Продажи апартаментов (сделки, ДДУ) по Well Обводный и Well Московский — лист "Апартаменты (проект)" в соответствующем файле "Сводная_*.xlsx", столбец даты "Дата ДДУ", площадь "Площадь по ДДУ", выручка (рублей) "Цена ДДУ"
...
- Уступки (переуступки прав) — лист "Уступки ВЕЛЛ" в "Сводная_Обводный 118.xlsx", столбец даты — "Дата Договора уступки", площадь "Площадь по ДДУ", выручка (рублей) "Цена сделки (ДБ + ДДУ +ДУ)"; уступки относятся только к проекту Well Обводный
...
- ЗАПРЕЩЕНО придумывать цифры, категории или значения которых нет в данных. Если поиск по значению вернул 0 строк — прямо сообщи: "В данных не найдено записей с таким значением". Не интерпретируй и не угадывай.
...
- "Выручка", "сумма продаж", "сколько рублей" → sum_column по столбцу ВЫРУЧКИ (не площади!):
  - Лист "Апартаменты (проект)" (Well Обводный, Well Московский): column='Цена ДДУ', date_column='Дата ДДУ'
  - Лист "Уступки ВЕЛЛ": column='Цена сделки (ДБ + ДДУ +ДУ)', date_column='Дата Договора уступки'
- "Сколько апартаментов в проекте всего", "сколько всего апартаментов" → НЕ складывай distinct-счётчики из разных листов — апартаменты из "Уступки ВЕЛЛ" частично пересекаются с "Апартаменты (проект)" (уступка = перепродажа уже купленного по ДДУ апарта). Загружай оба листа через load_sheet и считай объединение уникальных номеров апартаментов самостоятельно, либо сообщи: в проекте Well Обводный 1342 уникальных апартамента (1214 по ДДУ + 128 только в уступках)
- "Сколько апартаментов в продаже", "остатки в продаже", "свободные апартаменты" → используй файл "Остатки в продаже_28.02.2026.xlsx", лист "Остатки в продаже"; строка "Всего" содержит итоговое число (столбец "шт.") и площадь (столбец "кв.м") — это реальные свободные остатки, НЕ считай distinct по листам с ДДУ/уступками
